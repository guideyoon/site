...
class RewriteRequest(BaseModel):
    ...
    instruction: Optional[str] = (
        "Make this text more engaging and suitable for a blog post."
    )


@router.post("/rewrite")
async def rewrite_content(
    request: RewriteRequest, current_user: User = Depends(get_current_user)
):
    ...
    if provider == "openai":
        ...
        try:
            ...
            response = client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a helpful blog writing assistant. Rewrite the provided text to be natural, engaging, and suitable for a Naver Blog post in Korean.",
                    },
                    {
                        "role": "user",
            ...
            raise HTTPException(
                status_code=500, detail=f"OpenAI processing failed: {str(e)}"
            )
        ...
        if not api_key:
            raise HTTPException(
                status_code=400, detail="Perplexity API Key is required."
            )
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Perplexity processing failed: {str(e)}"
            )
