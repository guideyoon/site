...
docker-compose -f infra/docker-compose.yml up -d postgres redis worker beat
if %ERRORLEVEL% NEQ 0 (
...
start "NaverCafe Backend" cmd /k "cd /d %PROJECT_ROOT%\backend && uvicorn app.main:app --reload --host 0.0.0.0 --port 8001"

:: 3. 프론트엔드 서버 실행 (새 창)
echo [3/3] 프론트엔드 웹 서버를 시작합니다 (Port 3000)...
start "NaverCafe Frontend" cmd /k "cd /d %PROJECT_ROOT%\frontend && npm run dev -- -H 0.0.0.0 -p 3000"
